...
1. Navigate to Azure Machine Learning Studio, click on **Notebooks** and expand **Odl_user_xxxx(1)>Openai-model (2)** folder then click on **config.json (3)**.
...
1. Navigate to the **Notebooks**, open **create-services.py** file from the path **User/odl_user_<inject key="DeploymentID" enableCopy="false"/>/artifacts/openai-model**
...
1. Navigate to the **Notebooks**, open **dataextraction.py** file from the path **User/ode_user_<inject key="DeploymentID" enableCopy="false"/>/artifacts/openai-model**
