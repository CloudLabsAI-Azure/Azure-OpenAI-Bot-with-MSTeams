...
1. Launch the Edge web browser and proceed to the Azure Portal. If you're not already logged in, sign in with your Azure account

2. Search and select **Deploy from a custom template** from the Azure search bar
...
6. On  the **Custom deployment** tab specify the following settings (leave others with their defaults value) and select **Review + create**:  
...
8. After the app service got deployed, from the resource group select the Web app and from the overview tab copy app service name and paste it into notepad. You will need it in further exercise.
...
5. On  the **Custom deployment** tab specify the following settings (leave others with their defaults) and select **Review + create**:  
    ...
    | Bot Endpoint | **https://appServiceName.azurewebsites.net/api/messages** (replace appServiceName with the app name which you have copied in Task 2 step 8) |
    | App Type | **SingleTenant** |
    | App Id   | **Enter Application Id value from Service Principal Details** |
    | Tenant Id | **Enter Tenant Id value from Service Principal Details** |
...
1. Navigate back to the Visual Studio code and follow the below instructions to update the values in the appsettings.json file.
...
1. Replace the values in the **appsettings.json** file, you can obtain the values from **Environment Details** page, under **Service Principal Details**. 
...
1. Follow the below-mentioned instructions to deploy the web app:
    ...
    - From the Visual studio code, click on **Azure**
    - Expand the assigned **Subscription**
    - Right-click on **botapplication-<inject key="DeploymentID" enableCopy="false"/>** under App Services
...
### Task 4: Publish the chatbot on the Microsoft Teams channel

1. Navigate back to the Azure portal and from the Overview page of the resource group select Azure Bot **botapplication-<inject key="DeploymentID" enableCopy="false"/>**.
